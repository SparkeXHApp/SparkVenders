...
open class JXSegmentedTitleDataSource: JXSegmentedBaseDataSource{
    ...
    /// 是否展示分割线
    open var isShowSeparLineView: Bool = false
    /// 分割线背景色
    open var separLineColor: UIColor = UIColor.gray
    /// 分割线尺寸
    open var separLineSize: CGSize = CGSize.init(width: 2, height: 10)
    /// title与分割线之前的间距
    open var marginSpacing: CGFloat = 0
    ...
    open override func preferredRefreshItemModel( _ itemModel: JXSegmentedBaseItemModel, at index: Int, selectedIndex: Int) {
        ...
        myItemModel.isShowSeparLineView = isShowSeparLineView
        myItemModel.separLineColor = separLineColor
        myItemModel.separLineSize = separLineSize
        myItemModel.marginSpacing = marginSpacing
        ...
        if index == titles.count - 1 {
            myItemModel.isShowSeparLineView = false
        }
    }
    ...
    open override func preferredSegmentedView(_ segmentedView: JXSegmentedView, widthForItemAt index: Int) -> CGFloat {
        ...
        if itemWidth == JXSegmentedViewAutomaticDimension {
            width += (dataSource[index] as! JXSegmentedTitleItemModel).textWidth + marginSpacing*2
        }else {
            width += itemWidth + marginSpacing*2
        }
        return width
    }
