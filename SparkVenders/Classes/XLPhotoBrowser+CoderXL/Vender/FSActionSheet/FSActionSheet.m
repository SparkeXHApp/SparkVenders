...
- (void)tableView:(UITableView *)tableView willDisplayCell:(UITableViewCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 0) {
        ...
        for (FSActionSheetItem *item in _items) {
            if (item.image) {
                NSBundle *resourceBundle = [NSBundle bundleWithPath:[[NSBundle bundleForClass:[self class]] pathForResource:@"XLPhotoBrowser+CoderXL" ofType:@"bundle"]];
                cancelItem = FSActionSheetTitleWithImageItemMake(FSActionSheetTypeNormal, [UIImage imageNamed:@"FSActionSheet_cancel" inBundle:resourceBundle compatibleWithTraitCollection:nil], _cancelTitle);
                break;
            }
        }
