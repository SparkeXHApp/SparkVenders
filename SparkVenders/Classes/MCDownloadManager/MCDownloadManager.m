...
- (NSString *)filename {
    if (_filename == nil) {
        ...
        NSString *str1  = arr[0];
        NSString *pathExtension = str1.pathExtension;
...
- (void)setUrl:(NSString *)url {
    ...
    if ([_url isEqualToString:@""] && ![url isEqualToString:@""]) {
        _filename = nil;
        _filePath = nil;
...
- (MCDownloadReceipt *)wx_downloadFileWithURL:(NSString * _Nullable)url
                                        writeInfo:(NSDictionary *)info
    ...
    dispatch_sync(self.synchronizationQueue, ^{
        ...
        [receipt setSuccessBlock:^(NSURLRequest *request, NSHTTPURLResponse  * _Nullable response, NSURL *filePath) {
            
            //改成通知的方式。待测试
//            [self wx_MCWriteDownloadWithInfo:info];
            [[NSNotificationCenter defaultCenter] postNotificationName:@"WriteDownloadNotication" object:nil userInfo:info];
            success(request, response, filePath);
        }];
...
- (MCDownloadReceipt *)downloadReceiptForURL:(NSString *)url {
    ...
    MCDownloadReceipt *receipt = self.allDownloadReceipts[cacheKey];
    
    if (receipt) {
        NSLog(@"直接使用了已有的receipt");
        if (![realUrl isEqualToString:@""]) {
            [receipt setUrl:realUrl];
        }
        return receipt;
    }
    // 这个时候是拿不到url的
    receipt = [[MCDownloadReceipt alloc] initWithURL:realUrl];
    receipt.state = MCDownloadStateNone;
    receipt.totalBytesExpectedToWrite = 1;
    
    dispatch_sync(self.synchronizationQueue, ^{
        [self.allDownloadReceipts setObject:receipt forKey:cacheKey];
        [self saveReceipts:self.allDownloadReceipts];
    });
...
- (void)suspendAll {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:@"refreshSuspendData" object:self userInfo:nil];
...
- (void)suspendWithDownloadReceipt:(MCDownloadReceipt *)receipt {
    ...
    [self updateReceiptWithURL:task.taskDescription state:MCDownloadStateSuspened];
...
- (void)removeWithURL:(NSString *)url {
    ...
    MCDownloadReceipt *receipt = [self downloadReceiptForURL:url];
    [self removeWithDownloadReceipt:receipt key:cacheKey];
    
}
- (void)removeWithDownloadReceipt:(MCDownloadReceipt *)receipt key:(NSString*)cacheKey {
    ...
    dispatch_sync(self.synchronizationQueue, ^{
        [self.allDownloadReceipts removeObjectForKey:cacheKey];
        [self saveReceipts:self.allDownloadReceipts];
    });
