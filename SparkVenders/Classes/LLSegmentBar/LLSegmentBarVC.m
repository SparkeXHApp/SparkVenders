...
//    UIBezierPath *maskPath = [UIBezierPath bezierPathWithRoundedRect: self.segmentBar.bounds byRoundingCorners:UIRectCornerTopLeft | UIRectCornerTopRight cornerRadii:CGSizeMake(15,15)];
//    CAShapeLayer *maskLayer = [[CAShapeLayer alloc] init];
//    maskLayer.frame = self.segmentBar.bounds;
//    maskLayer.path = maskPath.CGPath;
//    self.segmentBar.layer.mask = maskLayer;
...
- (void)segmentBar:(LLSegmentBar *)segmentBar didSelectIndex:(NSInteger)toIndex fromIndex:(NSInteger)fromIndex{
    ...
    if ([self.delegate respondsToSelector:@selector(segmentBarVCToIndex:)]) {
        [self.delegate segmentBarVCToIndex:toIndex];
    }
}
